...
From my analysis of the SortingAlgorithm driver, I noticed a few issues, the largest being in the isSorted method in the SortingAlgorithmDriver since it uses CompareTo to check whether the prev is less than the curr, but this treats values as strings, thus "10" < "9" since 9 is greater than 1. This
causes the interpretation of ascending to be unsorted in Nullsort when it should be sorted. Another minor issue I noticed was in the descending.data file since every 11th data point is missing a thousand digit causing the data not be fully a descending data set. If we used the insertion sort function for example, 
it wouldn't display worst case behavior since for every value missing thousands place, the insertion sort algorithm has to do more work (traverse more further back in the sorted list since the hundreds unsorted value will always be less than the thousands place numbers).
