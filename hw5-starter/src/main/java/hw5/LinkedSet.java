...
public class LinkedSet<T> implements Set<T> {
  ...
  protected void remove(Node<T> target) {
    ...
    // if pNode and node !null we have deleted target node (> 2 nodes)

    if (head == target) {
      head = target.next;
    }
    // pNode is null and nNode !null, deletes head target (2 node)

    if (tail == target) {
      tail = target.prev;
    }
    // pNode !null and nNode null, deletes tail target (2 node)

    // if pNode and nNode are null set head and tail to null (1 node)
  }
